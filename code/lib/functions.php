...
function datacheck($field, $field_name, $field_data, $table, $auth_type){
	if(!empty($field_data)){
		$Query = "SELECT ".$field." FROM ".$table." WHERE ".$field_name." = '".$field_data."' ";
		$rs = mysqli_query($GLOBALS['conn'], $Query);
		if(mysqli_num_rows($rs) > 0){
			$row = mysqli_fetch_row($rs);
			$data_id = $row[0]; 
		} else {
			$data_id = getMaximum($table, $field);
			if($auth_type == 1){
				mysqli_query($GLOBALS['conn'], "INSERT ".$table." (".$field.", ".$field_name.", auth_type) VALUES ('".$data_id."', '".$field_data."', '1') ") or die(mysqli_error($GLOBALS['conn']));
			} else {
				mysqli_query($GLOBALS['conn'], "INSERT ".$table." (".$field.", ".$field_name.") VALUES ('".$data_id."', '".$field_data."') ") or die(mysqli_error($GLOBALS['conn']));
			}
